        ...
        <div class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-p-6">
            <div class="tw-flex tw-items-center tw-justify-between tw-mb-6">
                ...
                @if($payment->isPending())
                    <span class="tw-px-3 tw-py-1 tw-bg-yellow-100 tw-text-yellow-700 tw-rounded-full tw-text-sm tw-font-medium">Chờ thanh toán</span>
                @elseif($payment->isCompleted())
                    <span class="tw-px-3 tw-py-1 tw-bg-green-100 tw-text-green-700 tw-rounded-full tw-text-sm tw-font-medium">Đã thanh toán</span>
                @else
                    <span class="tw-px-3 tw-py-1 tw-bg-red-100 tw-text-red-700 tw-rounded-full tw-text-sm tw-font-medium">Thất bại</span>
                @endif
            </div>
            ...
            <div class="tw-space-y-4">
                ...
                <div class="tw-flex tw-justify-between">
                    ...
                    <span class="tw-font-medium tw-text-gray-800">{{ $payment->created_at->format('d/m/Y H:i') }}</span>
                </div>
                @if($payment->paid_at)
                    <div class="tw-flex tw-justify-between">
                        <span class="tw-text-gray-600">Ngày thanh toán</span>
                        <span class="tw-font-medium tw-text-gray-800">{{ $payment->paid_at->format('d/m/Y H:i') }}</span>
                    </div>
                @endif
            </div>
        </div>
        ...
        @if($payment->isPending() && $bankInfo)
            <div class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-p-6">
                ...
                <div class="tw-flex tw-justify-center tw-mb-4">
                    <img src="{{ $bankInfo['qr_url'] }}" alt="QR Code" class="tw-w-48 tw-h-48 tw-rounded-lg tw-border tw-border-gray-200">
                </div>
                ...
                <div class="tw-bg-gray-50 tw-rounded-xl tw-p-4 tw-mb-4">
                    <div class="tw-space-y-3 tw-text-sm">
                        ...
                        <div>
                            ...
                            <p class="tw-font-bold tw-text-lg tw-text-gray-800">{{ number_format($bankInfo['amount']) }}đ</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="tw-bg-yellow-50 tw-border tw-border-yellow-200 tw-rounded-xl tw-p-4 tw-mb-4">
                    <p class="tw-text-sm tw-text-yellow-700">
                        <strong>Lưu ý:</strong> Vui lòng ghi đúng nội dung chuyển khoản. Gói dịch vụ sẽ được kích hoạt tự động sau khi xác nhận thanh toán (thường trong vòng 5 phút).
                    </p>
                </div>
                ...
                <form action="{{ route('brands.payments.check', [$brand, $payment]) }}" method="POST">
                    @csrf
                    <button type="submit" class="tw-w-full tw-px-4 tw-py-3 tw-bg-[#16a249] tw-text-white tw-rounded-lg tw-font-medium hover:tw-bg-[#138a3e] tw-transition">
                        Tôi đã thanh toán - Kiểm tra trạng thái
                    </button>
                </form>
            </div>
        @else
            <div class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-p-6 tw-flex tw-flex-col tw-items-center tw-justify-center">
                @if($payment->isCompleted())
                    <div class="tw-w-16 tw-h-16 tw-bg-green-100 tw-rounded-full tw-flex tw-items-center tw-justify-center tw-mb-4">
                        <svg class="tw-w-8 tw-h-8 tw-text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                    </div>
                    ...
                    <a href="{{ route('brands.subscription.show', $brand) }}" class="tw-px-4 tw-py-2 tw-bg-[#16a249] tw-text-white tw-rounded-lg tw-font-medium hover:tw-bg-[#138a3e] tw-transition">
                        Xem gói dịch vụ
                    </a>
                @endif
            </div>
