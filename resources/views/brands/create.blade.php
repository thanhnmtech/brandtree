<x-app-layout>
    ...
    <div class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-p-6 md:tw-p-8">
        <form action="{{ route('brands.store') }}" method="POST" enctype="multipart/form-data" class="tw-max-w-2xl">
            @csrf
            ...
            <div class="tw-mb-6">
                ...
                <input type="text" name="industry" id="industry" value="{{ old('industry') }}"
                       class="tw-w-full tw-px-4 tw-py-3 tw-border tw-border-gray-300 tw-rounded-lg focus:tw-ring-2 focus:tw-ring-[#16a249] focus:tw-border-transparent tw-transition"
                       placeholder="VD: Công nghệ, Thực phẩm, Thời trang...">
                @error('industry')
                    <p class="tw-text-red-500 tw-text-sm tw-mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="tw-mb-6">
                ...
                <input type="text" name="target_market" id="target_market" value="{{ old('target_market') }}"
                       class="tw-w-full tw-px-4 tw-py-3 tw-border tw-border-gray-300 tw-rounded-lg focus:tw-ring-2 focus:tw-ring-[#16a249] focus:tw-border-transparent tw-transition"
                       placeholder="VD: Việt Nam, Đông Nam Á, Toàn cầu...">
                @error('target_market')
                    <p class="tw-text-red-500 tw-text-sm tw-mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="tw-mb-6">
                ...
                <textarea name="description" id="description" rows="4"
                          class="tw-w-full tw-px-4 tw-py-3 tw-border tw-border-gray-300 tw-rounded-lg focus:tw-ring-2 focus:tw-ring-[#16a249] focus:tw-border-transparent tw-transition tw-resize-none"
                          placeholder="Mô tả về thương hiệu của bạn...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="tw-text-red-500 tw-text-sm tw-mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="tw-flex tw-items-center tw-gap-4">
                ...
                <a href="{{ route('brands.index') }}" class="tw-px-6 tw-py-3 tw-border tw-border-gray-300 tw-text-gray-700 tw-rounded-lg tw-font-medium hover:tw-bg-gray-50 tw-transition">
                    Hủy
                </a>
            </div>
        </form>
