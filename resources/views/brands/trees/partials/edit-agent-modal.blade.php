<div x-show="openEditModal" style="display: none;" x-data="{
        ...
        openEdit(id, name, hasKnowledge, instruction) {
            this.agentId = id;
            this.agentName = name;
        ...
        async saveAgent() {
            ...
            try {
                ...
                if (result.status === 'success') {
                    // Cập nhật UI trực tiếp thay vì reload
                    const card = document.querySelector(`[data-agent-id='${this.agentId}']`);
                    if (card) {
                        card.querySelector('.agent-name').textContent = this.agentName;
                        card.querySelector('.agent-instruction').textContent = this.instruction || 'Chưa có mô tả chi tiết cho agent này.';
                    }
